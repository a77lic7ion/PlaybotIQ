  ...
  try {
    ...
    if (groundingChunks) {
        references = groundingChunks
            ...
            .filter((ref: Reference) => ref.uri); // Filter out any chunks without a URI
    }
    
    // Deduplicate references based on URI
    const uniqueReferences = Array.from(new Map(references.map(item => [item['uri'], item])).values());

    return { guide, references: uniqueReferences };

  } catch (error) {
    console.error("Error generating content from Gemini API:", error);
    throw new Error(
      "Failed to get a response from the AI. Please check your API key and network connection."
    );
  }
}